...
function [keys RT] = recordKeysNoBT(startTime,duration,deviceNumber,ignoreKey,exitIfKey)
...
% Don't start until keys are released - don't want this for scanning
% potentially could trhow off time
% if IsOSX
...
while 1
    ...
    if keyIsDown
        tempkey = KbName(keyCode);
        if ~strcmp(tempkey,ignoreKey)
            keys = [keys tempkey(1)];%change back to (keyCode)?
